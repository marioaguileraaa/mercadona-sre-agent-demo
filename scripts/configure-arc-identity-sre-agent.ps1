...
$skill = @{
    ...
    properties = @{
        ...
        skillContent = @'
Use only the existing ArcBox workspace law-arcbox-demo-001 and Arc machines ArcBox-Win2K22 and ArcBox-Win2K25. Real signals are Azure Arc connectivity, Azure Monitor Agent health, Heartbeat, existing VM Insights metrics in InsightsMetrics, and filtered host System/Application events. Performance comes from the preserved MSVMI-ama-vmi-default-dcr; the additive identity DCR is events-only, and an empty Perf table is expected. Events from source Mercadona.IdentityOps with demoSynthetic=true and event IDs 4101/4102 are synthetic identity-service demonstrations, never evidence of genuine AD FS or domain-controller activity.

Use the aggregate-only queries under kql/arc-identity: fleet-heartbeat.kql, data-freshness.kql, synthetic-token-failure-burst.kql, performance-correlation.kql, extension-health.arg.kql, and change-tracking.kql. Change tracking is optional and must be reported as unavailable when the existing ArcBox environment does not provide its table; never enable it autonomously. The expected ArcBox operating window starts at 08:20 Europe/Madrid, after startup grace, and ends at the fixed 18:00 UTC auto-shutdown; Heartbeat or InsightsMetrics gaps outside that window are expected and must not be treated as incidents. Default reports must not include user names, event-message samples, authentication material, or raw RenderedDescription values.
